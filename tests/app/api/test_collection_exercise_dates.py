...
class TestCollectionExerciseDates(AppContextTestCase):
    ...
    def test_collection_exercises_event_dates_success(self):
        ...
        with self.app.app_context():
            ...
            response = self.test_client.get(f'dashboard/collectionexercises/event/go_live'
                                            f'?collectionExerciseId={first_collex_id}'
                                            f'&collectionExerciseId={second_collex_id}')
    ...
    def test_collection_exercises_event_dates_malformed_id_responds_400(self):
        with self.app.app_context():
            response = self.test_client.get(f'dashboard/collectionexercises/event/go_live'
                                            f'?collectionExerciseId=not-a-valid-uuid')
