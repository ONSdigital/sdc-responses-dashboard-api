...
class TestCollectionExerciseEventDates(AppContextTestCase):
    ...
    def test_get_two_collection_exercises_event_dates_success(self):
        ...
        with self.app.app_context():
            responses.add(responses.GET, (f'{self.app.config["COLLECTION_EXERCISE_URL"]}'
                                          f'/collectionexercises/{first_collection_exercise_id}'
                                          f'/events/go_live'), json=first_collection_exercise_go_live_response,
                          status=200)
            responses.add(responses.GET, (f'{self.app.config["COLLECTION_EXERCISE_URL"]}'
                                          f'/collectionexercises/{second_collection_exercise_id}'
                                          f'/events/go_live'), json=second_collection_exercise_go_live_response,
                          status=200)
            actual_timestamps = get_collection_exercises_event_dates(
                [first_collection_exercise_id, second_collection_exercise_id], event_tag='go_live')

        self.assertEqual({first_collection_exercise_id: first_collection_exercise_go_live_response['timestamp'],
    ...
    def test_get_two_collection_exercises_event_dates_with_one_missing_timestamp(self):
        ...
        with self.app.app_context():
            responses.add(responses.GET, (f'{self.app.config["COLLECTION_EXERCISE_URL"]}'
                          ...
                          status=200)
            responses.add(responses.GET, (f'{self.app.config["COLLECTION_EXERCISE_URL"]}'
                                          f'/collectionexercises/{collection_exercise_id_missing_timestamp}'
                                          f'/events/go_live'), json=go_live_missing_timestamp_response, status=200)
            actual_timestamps = get_collection_exercises_event_dates(
                [first_collection_exercise_id, collection_exercise_id_missing_timestamp], event_tag='go_live')

        self.assertEqual({first_collection_exercise_id: first_collection_exercise_go_live_response['timestamp'],
                          collection_exercise_id_missing_timestamp: None}, actual_timestamps)
    ...
    def test_get_two_collection_exercises_event_dates_with_one_error_response(self):
        ...
        with self.app.app_context():
            ...
            responses.add(responses.GET, (f'{self.app.config["COLLECTION_EXERCISE_URL"]}'
                                          f'/collectionexercises/{second_collection_exercise_id}'
                                          f'/events/go_live'), status=500)
            actual_timestamps = get_collection_exercises_event_dates(
                [first_collection_exercise_id, second_collection_exercise_id], event_tag='go_live')

        self.assertEqual({first_collection_exercise_id: first_collection_exercise_go_live_response['timestamp'],
                          second_collection_exercise_id: None}, actual_timestamps)
    ...
    def test_get_two_collection_exercises_event_dates_with_one_exception_in_controller(self):
        ...
        self.assertEqual(
            {first_collection_exercise_id: '2018-12-14T00:00:00.000Z', second_collection_exercise_id: None},
            actual_timestamps)
